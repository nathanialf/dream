...
That is the "mode/level select" feature the task description hinted at, and it
reaches `game_mode` 1 and 2 (never reached by `title_start_right.txt` alone)
without knowing where an in-level trigger for them is.
...
- **title_start_right.txt** (+74): the whole non-title-specific gameplay bring-up
  path (reset, PPU/SPC init, entity/animation update, mode-0 particle draw, sound
