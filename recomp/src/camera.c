...
void camera_follow_player(SnesState* ss) {
  ...
  if(a != 0x0001) {
    ...
    a = alu_adc16(ss, a, t_read16_dp(ss, dp, camera_y_bias));
    S(0xA1EB, 2);                           /* C0A1EB sta camera_y_lookahead */
    t_write16_dp(ss, dp, camera_y_lookahead, a);
