...
void clear_sprite_table(SnesState* ss) {
  ...
  for(int i = 0; i < 16; i++) {             /* C0A500..C0A52D stz $0400,+2 */
    ...
    /* not an index: the 16 stz instructions each carry their own absolute
     * operand, so there is no index add to carry into the bank (L22). */
    t_write16(ss, ss_abs(ss, (uint16_t) (oam_buffer_upper + i * 2)), 0);
  }
  SIMM16(0xA530);                           /* C0A530 lda #$0200 */
  a = 0x0200;
  ss_set_nz16(ss, a);
  S(0xA533, 2);                             /* C0A533 sta oam_write_ptr */
  t_write16_dp(ss, dp, oam_write_ptr, a);
  S(0xA535, 2);                             /* C0A535 stz entity_render_index */
  t_write16_dp(ss, dp, entity_render_index, 0);
  ss_set_a(ss, a);
...
void oam_hide_unused_sprites(SnesState* ss) {
  ...
  ss_set_nz16(ss, x);
  SIMM16(0xADE9); alu_cpx16(ss, x, 0x0400); /* C0ADE9 cpx #$0400 */
  S(0xADEC, 1); t_branch(ss, x == 0x0400);/* C0ADEC beq loc_C0ADFC */
  if(x != 0x0400) {
    SIMM16(0xADEE);                         /* C0ADEE lda #$F0FF */
    a = 0xF0FF;
    ss_set_nz16(ss, a);
    do {
      ...
      SIMM16(0xADF7); alu_cpx16(ss, x, 0x0400);  /* C0ADF7 cpx #$0400 */
...
void oam_dma_upload(SnesState* ss) {
  ...
  SIMM16(0xAE06); a = 0x0220; ss_set_nz16(ss, a);  /* C0AE06 lda #$0220 */
  S(0xAE09, 3); t_write16(ss, ss_abs(ss, DASL0), a);  /* C0AE09 sta DASL0 */
  SIMM16(0xAE0C); a = 0x0400; ss_set_nz16(ss, a);  /* C0AE0C lda #$0400 */
  ...
  SIMM16(0xAE19); a = 0x0001; ss_set_nz16(ss, a);  /* C0AE19 lda #$0001 */
  S(0xAE1C, 2); t_write16_dp(ss, dp, dma_pending_mask, a); /* C0AE1C sta $02 */
  ss_set_a(ss, a);
...
void entity_sort_draw_order(SnesState* ss) {
  ...
  a = t_read16(ss, ss_abs(ss, (entity_depth_key + y)));
  ss_set_nz16(ss, a);
  S(0xAE26, 2); t_write16_dp(ss, dp, depth_sort_key, a); /* C0AE26 sta $52 */
  SIMM16(0xAE28);                           /* C0AE28 ldx #$09A6 */
  x = 0x09A6;
  ss_set_nz16(ss, x);
  ...
  for(;;) {
    ...
    y = t_read16_dp(ss, dp, ptr_04 + x);
    ss_set_nz16(ss, y);
    S(0xAE2D, 3); t_index(ss);              /* C0AE2D lda entity_depth_key,Y */
    a = t_read16(ss, ss_abs(ss, (entity_depth_key + y)));
    ss_set_nz16(ss, a);
    S(0xAE30, 2);                           /* C0AE30 cmp $52 */
    alu_cmp16(ss, a, t_read16_dp(ss, dp, depth_sort_key));
    S(0xAE32, 2); t_write16_dp(ss, dp, depth_sort_key, a); /* C0AE32 sta $52 */
    ...
    if(ge) {
      ...
      if(eq) {
        ...
        if(front) {
          ...
          y = t_read16_dp(ss, dp, ptr_04 + x);
          ss_set_nz16(ss, y);
          SIMM16(0xAE72); alu_cmp16(ss, a, 0x0004);  /* C0AE72 cmp #$0004 */
          S(0xAE75, 1); t_branch(ss, a == 0x0004);  /* C0AE75 beq loc_C0AE3E */
          if(a != 0x0004) {
            SIMM16(0xAE77); alu_cmp16(ss, a, 0x0002);  /* C0AE77 cmp #$0002 */
  ...
  for(;;) {
    ...
    y = t_read16_dp(ss, dp, nmi_handler_ptr + x);
    ss_set_nz16(ss, y);
    ...
    alu_cmp16(ss, a, t_read16_dp(ss, dp, depth_sort_key));
    S(0xAE4C, 2); t_write16_dp(ss, dp, depth_sort_key, a); /* C0AE4C sta $52 */
    ...
    SIMM16(0xAE58); alu_cpx16(ss, x, 0x09A6);  /* C0AE58 cpx #$09A6 */
...
void entity_upload_pending_tiles(SnesState* ss) {
  ...
  SIMM8(0xAE86); y = 0x80; ss_set_nz8(ss, (uint8_t) y); /* C0AE86 ldy #$80 */
  S(0xAE88, 3);                             /* C0AE88 sty VMAIN */
  t_write8(ss, ss_abs(ss, VMAIN), (uint8_t) y);
  SIMM8(0xAE8B); y = 0x01; ss_set_nz8(ss, (uint8_t) y); /* C0AE8B ldy #$01 */
  ...
  for(;;) {
    ...
    a = t_read16(ss, ss_abs(ss, (entity_tile_job + 6 + x)));
    ss_set_nz16(ss, a);
    ...
    a = t_read16(ss, ss_abs(ss, (entity_tile_job + 0 + x)));
    ss_set_nz16(ss, a);
    ...
    a = t_read16(ss, ss_abs(ss, (entity_tile_job + 2 + x)));
    ss_set_nz16(ss, a);
    ...
    a = t_read16(ss, ss_abs(ss, (entity_tile_job + 4 + x)));
    ss_set_nz16(ss, a);
    ...
    SIMM16(0xAEB1); a = alu_adc16(ss, a, 0x0008);  /* C0AEB1 adc #$0008 */
