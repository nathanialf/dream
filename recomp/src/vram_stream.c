...
void vram_write_tile_row_planes(SnesState* ss) {
  ...
  for(;;) {
    ...
    a = alu_ora16(ss, a, t_read16_dp(ss, dp, ptr_04 + 2));
    S(0x9C37, 3);                           /* C09C37 sta VMDATAL */
    t_write16(ss, ss_abs(ss, VMDATAL), a);
    SIMM16(0x9C3A); a = 0x0007; ss_set_nz16(ss, a);   /* C09C3A lda #$0007 */
    do {
    ...
    t_write16(ss, ss_abs(ss, VMDATAL), a);
    SIMM16(0x9C52); a = 0x0007; ss_set_nz16(ss, a);   /* C09C52 lda #$0007 */
    do {
    ...
    SIMM16(0x9C5C); alu_cmp16(ss, y, 0x0010); /* C09C5C cpy #$0010 */
...
void vram_generate_particle_tile(SnesState* ss) {
  ...
  SIMM16(0x9C25); y = 0x000F; ss_set_nz16(ss, y);   /* C09C25 ldy #$000F */
...
void vram_upload_shared_tileset_c5(SnesState* ss) {
  ...
  SIMM16(0x9234); a = 0x0000; ss_set_nz16(ss, a);   /* C09234 lda #$0000 */
  S(0x9237, 3);                             /* C09237 sta VMADDL */
  t_write16(ss, ss_abs(ss, VMADDL), a);
  SIMM16(0x923A); x = 0x00C5; ss_set_nz16(ss, x);   /* C0923A ldx #$00C5 */
  SIMM16(0x923D); a = 0x02C0; ss_set_nz16(ss, a);   /* C0923D lda #$02C0 */
  SIMM16(0x9240); y = 0x0C00; ss_set_nz16(ss, y);   /* C09240 ldy #$0C00 */
...
void vram_stream_descriptor_dispatch(SnesState* ss) {
  ...
  if(!selected) {
    ...
    SIMM16(0x9C6C); a = alu_and16(ss, a, 0x00FF);  /* C09C6C and #$00FF */
  ...
  if(!continuing) {
    ...
    SIMM16(0x9CB0); a = alu_adc16(ss, a, 0x0008);  /* C09CB0 adc #$0008 */
    ...
    SIMM16(0x9CE1); a = alu_adc16(ss, a, 0x0008);  /* C09CE1 adc #$0008 */
  ...
  SIMM16(0x9CE7); y = 0x0400; ss_set_nz16(ss, y);   /* C09CE7 ldy #$0400 */
  ...
  if(!done) {
    ...
    SIMM16(0x9CF0); a = alu_sbc16(ss, a, 0x0400);  /* C09CF0 sbc #$0400 */
    const bool more = (a & 0x8000) == 0;
    S(0x9CF3, 1); t_branch(ss, more);       /* C09CF3 bpl loc_C09CFB */
    if(!more) {
      SIMM16(0x9CF5); a = alu_eor16(ss, a, 0xFFFF);  /* C09CF5 eor #$FFFF */
...
void build_metatile_column_500(SnesState* ss) {
  ...
  a = t_read16_dp(ss, dp, camera_x);
  ss_set_nz16(ss, a);
  SIMM16(0x9E85); a = alu_and16(ss, a, 0xFFE0);   /* C09E85 and #$FFE0 */
  S(0x9E88, 2);                             /* C09E88 ldy $82 */
  y = t_read16_dp(ss, dp, 0x0082);
  ss_set_nz16(ss, y);
  ...
  if(!ahead) {
    ...
    SIMM16(0x9EAB); a = alu_adc16(ss, a, 0x00E0);  /* C09EAB adc #$00E0 */
  }

  /* loc_C09EAE */
  SI(0x9EAE); y = a; ss_set_nz16(ss, y);    /* C09EAE tay */
  SIMM16(0x9EAF); a = alu_and16(ss, a, 0xFFE0);  /* C09EAF and #$FFE0 */
  ...
  a = alu_adc16(ss, a, t_read16_dp(ss, dp, scratch_18));
  S(0x9EB8, 2);                             /* C09EB8 sta $18 */
  t_write16_dp(ss, dp, scratch_18, a);
  SI(0x9EBA); a = y; ss_set_nz16(ss, a);    /* C09EBA tya */
  SIMM16(0x9EBB); a = alu_and16(ss, a, 0x0018);  /* C09EBB and #$0018 */
  S(0x9EBE, 2);                             /* C09EBE adc tilemap_b_addr */
  a = alu_adc16(ss, a, t_read16_dp(ss, dp, tilemap_b_addr));
  S(0x9EC0, 2);                             /* C09EC0 sta $1C */
  t_write16_dp(ss, dp, scratch_18 + 4, a);
  SI(0x9EC2); a = y; ss_set_nz16(ss, a);    /* C09EC2 tya */
  SIMM16(0x9EC3); a = alu_and16(ss, a, 0x0018);  /* C09EC3 and #$0018 */
  SIMM16(0x9EC6); a = alu_eor16(ss, a, 0x0018);  /* C09EC6 eor #$0018 */
  S(0x9EC9, 2);                             /* C09EC9 adc tilemap_b_addr */
  a = alu_adc16(ss, a, t_read16_dp(ss, dp, tilemap_b_addr));
  S(0x9ECB, 2);                             /* C09ECB sta $1E */
  t_write16_dp(ss, dp, scratch_18 + 6, a);

  S(0x9ECD, 1);                             /* C09ECD phk */
  ss_idle(ss);
  ss_check_int(ss);
  ss_push8(ss, pb);
  SIMM16(0x9ECE); x = 0x06C0; ss_set_nz16(ss, x);  /* C09ECE ldx #$06C0 */

  for(;;) {
    /* loc_C09ED1 */
    S(0x9ED1, 2);                           /* C09ED1 lda metatile_data_bank */
    a = t_read16_dp(ss, dp, metatile_data_bank);
    ss_set_nz16(ss, a);
    ...
    a = t_read16(ss, t_idl_ptr(ss, t_dp(dp, scratch_18)));
    ss_set_nz16(ss, a);
    const bool vflip = (a & 0x8000) != 0;
    S(0x9ED8, 1); t_branch(ss, vflip);      /* C09ED8 bmi loc_C09F39 */

    if(!vflip) {
      SIMM16(0x9EDA); alu_bit_imm16(ss, a, 0x4000);  /* C09EDA bit #$4000 */
      ...
      if(!hflip) {
        ...
        a = t_read16(ss, ss_abs(ss, (ptr_04 + y)));
        ss_set_nz16(ss, a);
        S(0x9EF4, 2); t_index(ss);          /* C09EF4 sta ptr_04,X */
        t_write16_dp(ss, dp, ptr_04 + x, a);
        S(0x9EF6, 3); t_index(ss);          /* C09EF6 lda $0006,Y */
        a = t_read16(ss, ss_abs(ss, (0x0006 + y)));
        ss_set_nz16(ss, a);
      } else {
  ...
  do {
    ...
    alu_cpx16(ss, x, t_read16_dp(ss, dp, scratch_18 + 4));
...
void build_metatile_column_580(SnesState* ss) {
  ...
  a = t_read16_dp(ss, dp, layer_parallax_mode);
  ss_set_nz16(ss, a);
  const bool ahead = (a & 0x8000) == 0;
  ...
  if(!ahead) {
    ...
    SIMM16(0x9FC2); a = alu_adc16(ss, a, 0x0100);  /* C09FC2 adc #$0100 */
  }

  /* loc_C09FC5 */
  SI(0x9FC5); y = a; ss_set_nz16(ss, y);    /* C09FC5 tay */
  SIMM16(0x9FC6); a = alu_and16(ss, a, 0xFFE0);  /* C09FC6 and #$FFE0 */
  S(0x9FC9, 2);                             /* C09FC9 ldx $82 */
  x = t_read16_dp(ss, dp, 0x0082);
  ss_set_nz16(ss, x);
  ...
  a = t_read16_dp(ss, dp, camera_y);
  ss_set_nz16(ss, a);
  SIMM16(0x9FE3); a = alu_and16(ss, a, 0xFFE0);  /* C09FE3 and #$FFE0 */
  ...
  SIMM16(0x9FF0); a = alu_and16(ss, a, 0x0018);  /* C09FF0 and #$0018 */
  ...
  SIMM16(0x9FFA); a = alu_and16(ss, a, 0x0018);  /* C09FFA and #$0018 */
  SIMM16(0x9FFD); a = alu_eor16(ss, a, 0x0018);  /* C09FFD eor #$0018 */
  ...
  ss_check_int(ss);
  ss_push8(ss, pb);
  SIMM16(0xA007); x = 0x06C0; ss_set_nz16(ss, x);  /* C0A007 ldx #$06C0 */
  ...
  for(;;) {
    ...
    if(!vflip) {
      SIMM16(0xA013); alu_bit_imm16(ss, a, 0x4000);  /* C0A013 bit #$4000 */
      const bool hflip = !ss_z(ss);
    ...
    SIMM16(0xA037); a = alu_adc16(ss, a, 0x0002);  /* C0A037 adc #$0002 */
    S(0xA03A, 2);                           /* C0A03A sta $18 */
    t_write16_dp(ss, dp, scratch_18, a);
    SI(0xA03C); a = x; ss_set_nz16(ss, a);  /* C0A03C txa */
    SI(0xA03D); ss_set_c(ss, false);        /* C0A03D clc */
    SIMM16(0xA03E); a = alu_adc16(ss, a, 0x0008);  /* C0A03E adc #$0008 */
    SI(0xA041); x = a; ss_set_nz16(ss, x);  /* C0A041 tax */
    SIMM16(0xA042); alu_cmp16(ss, a, 0x0708); /* C0A042 cmp #$0708 */
    const bool more = a != 0x0708;
  ...
  do {
    ...
    t_write16(ss, ss_abs(ss, (0x0580 + y)), a);
    SI(0xA0E1); a = y; ss_set_nz16(ss, a);  /* C0A0E1 tya */
    SI(0xA0E2); ss_set_c(ss, false);        /* C0A0E2 clc */
    SIMM16(0xA0E3); a = alu_adc16(ss, a, 0x0002);  /* C0A0E3 adc #$0002 */
    SIMM16(0xA0E6); a = alu_and16(ss, a, 0x003E);  /* C0A0E6 and #$003E */
...
void vram_upload_column_580(SnesState* ss) {
  ...
  if(!ahead) {
    ...
    SIMM16(0xA105); a = alu_adc16(ss, a, 0x0100);  /* C0A105 adc #$0100 */
  }
  ...
  SIMM16(0xA10B); a = alu_and16(ss, a, 0x003F);  /* C0A10B and #$003F */
  SIMM16(0xA10E); alu_bit_imm16(ss, a, 0x0020);  /* C0A10E bit #$0020 */
  const bool second = !ss_z(ss);
  SI(0xA111); ss_set_c(ss, false);          /* C0A111 clc */
  S(0xA112, 1); t_branch(ss, !second);      /* C0A112 beq loc_C0A117 */
  if(second) {
    SIMM16(0xA114); a = alu_adc16(ss, a, 0x03E0);  /* C0A114 adc #$03E0 */
  }
  /* loc_C0A117 */
  SIMM16(0xA117); a = alu_adc16(ss, a, 0x7800);  /* C0A117 adc #$7800 */
  S(0xA11A, 3); t_write16(ss, ss_abs(ss, VMADDL), a);  /* C0A11A sta VMADDL */
  SIMM16(0xA11D); a = 0x0580; ss_set_nz16(ss, a);      /* C0A11D lda #$0580 */
  S(0xA120, 3); t_write16(ss, ss_abs(ss, A1TL0), a);   /* C0A120 sta A1TL0 */
  S(0xA123, 3); t_write16(ss, ss_abs(ss, A2AL0), a);   /* C0A123 sta A2AL0 */
  SIMM16(0xA126); a = 0x0040; ss_set_nz16(ss, a);      /* C0A126 lda #$0040 */
  S(0xA129, 3); t_write16(ss, ss_abs(ss, DASL0), a);   /* C0A129 sta DASL0 */
  SIMM16(0xA12C); a = 0x1801; ss_set_nz16(ss, a);      /* C0A12C lda #$1801 */
