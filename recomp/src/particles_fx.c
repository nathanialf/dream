...
void mode1_particle_dispatch(SnesState* ss) {
  ...
  S(0x922D, 3);                             /* C0922D jmp loc_C09679 */
...
void mode2_particle_dispatch(SnesState* ss) {
  ...
  S(0x9230, 3);                             /* C09230 jmp loc_C097DD */
...
void particle_spawn_mode0_weather(SnesState* ss) {
  ...
  S(0x9256, 3); x = 0x004E; ss_set_nz16(ss, x);  /* C09256 ldx #$004E */
  ...
  for(;;) {
    ...
    if(!taken) {
      S(0x925F, 3);                         /* C0925F jmp loc_C092EB */
      goto next_slot;
    }
    ...
    S(0x9267, 3); a = alu_and16(ss, a, 0x003F);    /* C09267 and #$003F */
    S(0x926A, 2); t_write16(ss, dp + ptr_04, a);   /* C0926A sta ptr_04 */
    S(0x926C, 3); a = alu_eor16(ss, a, 0xFFFF);    /* C0926C eor #$FFFF */
    ...
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x92BB, 3); a = alu_and16(ss, a, 0x7FFF);    /* C092BB and #$7FFF */
    S(0x92BE, 2);                           /* C092BE bit init_magic_FFFF */
    alu_bit16(ss, a, t_read16(ss, dp + init_magic_FFFF));
    taken = !ss_n(ss);
    S(0x92C0, 1); t_branch(ss, taken);      /* C092C0 bpl loc_C092C6 */
    if(!taken) {
      S(0x92C2, 3); a = alu_eor16(ss, a, 0xFFFF);  /* C092C2 eor #$FFFF */
    ...
    if(!taken) {
      ...
      a = t_read16(ss, dp + 0x9D); ss_set_nz16(ss, a);
      S(0x92CE, 3); a = alu_and16(ss, a, 0x0003);  /* C092CE and #$0003 */
      ...
      if(!taken) {
        S(0x92D6, 3); a = alu_eor16(ss, a, 0xFFFF);/* C092D6 eor #$FFFF */
    ...
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x92E0, 3); a = alu_and16(ss, a, 0x0010);    /* C092E0 and #$0010 */
    SI(0x92E3); ss_set_c(ss, false);               /* C092E3 clc */
    S(0x92E4, 3); a = alu_adc16(ss, a, 0x0010);    /* C092E4 adc #$0010 */
  ...
  next_slot:                                /* loc_C092EB */
    ...
    if(taken) break;
    S(0x92EF, 3);                           /* C092EF jmp loc_C09259 */
  }
...
void mode1_reset_particles_and_oam(SnesState* ss) {
  ...
  S(0x92F5, 2); t_write16(ss, dp + camera_x, 0);          /* C092F5 stz camera_x */
  S(0x92F7, 2); t_write16(ss, dp + camera_y, 0);          /* C092F7 stz camera_y */
  S(0x92F9, 2); t_write16(ss, dp + level_height_mask, 0); /* C092F9 stz level_height_mask */
  S(0x92FB, 3); t_write16(ss, ss_abs(ss, 0x0C1F), 0);     /* C092FB stz $0C1F */

  S(0x92FE, 3); a = 0x0001; ss_set_nz16(ss, a);           /* C092FE lda #$0001 */
  S(0x9301, 3); t_write16(ss, ss_abs(ss, 0x0C21), a);     /* C09301 sta $0C21 */
  S(0x9304, 3); a = 0x00FF; ss_set_nz16(ss, a);           /* C09304 lda #$00FF */
  S(0x9307, 3); t_write16(ss, ss_abs(ss, 0x0C23), a);     /* C09307 sta $0C23 */
  S(0x930A, 3); t_write16(ss, ss_abs(ss, 0x0C29), a);     /* C0930A sta $0C29 */
  S(0x930D, 3); a = 0x0020; ss_set_nz16(ss, a);           /* C0930D lda #$0020 */
  ...
  a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
  S(0x931B, 3); a = alu_and16(ss, a, 0x3000);             /* C0931B and #$3000 */
  S(0x931E, 3); a = alu_ora16(ss, a, 0x0E00);             /* C0931E ora #$0E00 */
...
static void particle_draw_finish(SnesState* ss, uint8_t pb, uint16_t base,
  ...
  t_write16(ss, dp + oam_write_ptr, y);
  S(base + 0x02, 2);                        /* C093F6 lda $18 */
  a = t_read16(ss, dp + scratch_18); ss_set_nz16(ss, a);
...
void particle_update_and_draw_mode0(SnesState* ss) {
  ...
  S(0x9332, 2); t_write16(ss, dp + scratch_18, a); /* C09332 sta $18 */
  S(0x9334, 2);                             /* C09334 lda camera_y */
  a = t_read16(ss, dp + camera_y); ss_set_nz16(ss, a);
  S(0x9336, 3); a = alu_and16(ss, a, 0x00FF);      /* C09336 and #$00FF */
  S(0x9339, 2); t_write16(ss, dp + 0x1A, a);       /* C09339 sta $1A */
  S(0x933B, 2);                             /* C0933B lda level_height_mask */
  a = t_read16(ss, dp + level_height_mask); ss_set_nz16(ss, a);
  SI(0x933D); ss_set_c(ss, false);                 /* C0933D clc */
  S(0x933E, 3); a = alu_adc16(ss, a, 0x00F0);      /* C0933E adc #$00F0 */
  S(0x9341, 2); t_write16(ss, dp + 0x1C, a);       /* C09341 sta $1C */
  S(0x9343, 3); x = 0x004E; ss_set_nz16(ss, x);    /* C09343 ldx #$004E */
  S(0x9346, 2);                             /* C09346 ldy oam_write_ptr */
  y = t_read16(ss, dp + oam_write_ptr); ss_set_nz16(ss, y);
  ...
  for(;;) {
    ...
    if(!taken) {
      S(0x934E, 3);                         /* C0934E jmp loc_C093FA */
      goto integrate;
    }
    ...
    if(!taken) {
      S(0x935B, 3);                         /* C0935B jmp loc_C093ED */
      goto next_slot;
    }
    ...
    t_write8(ss, dp + sprite_frame_bank, (uint8_t) a);
    ...
    a = (uint16_t) ((a & 0xff00) | b); ss_set_nz8(ss, b);
    S(0x9370, 2); t_write8(ss, dp + 0x9D, (uint8_t) a);  /* C09370 sta $9D */
    S(0x9372, 2);                           /* C09372 lda sprite_frame_bank */
    b = t_read8(ss, dp + sprite_frame_bank);
    a = (uint16_t) ((a & 0xff00) | b); ss_set_nz8(ss, b);
    S(0x9374, 2); t_write8(ss, dp + 0x9F, (uint8_t) a);  /* C09374 sta $9F */
    ...
    a = t_read16(ss, dp + 0x9F); ss_set_nz16(ss, a);
    S(0x93AA, 3); a = alu_and16(ss, a, 0x0030);     /* C093AA and #$0030 */
    S(0x93AD, 3); alu_cmp16(ss, a, 0x0030);         /* C093AD cmp #$0030 */
    taken = !ss_c(ss);
    ...
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x93CA, 3); a = alu_and16(ss, a, 0x7FFF);     /* C093CA and #$7FFF */
  ...
  next_slot:                                /* loc_C093ED */
    ...
    S(0x93F1, 3);                           /* C093F1 jmp loc_C09348 */
    continue;
  ...
  integrate:                                /* loc_C093FA */
    ...
    if(!taken) {
      S(0x9411, 2); t_write16(ss, dp + ptr_04, a);  /* C09411 sta ptr_04 */
      ...
      if(!taken) {
        ...
        a = t_read16(ss, dp + ptr_04); ss_set_nz16(ss, a);
        S(0x941B, 3); a = alu_eor16(ss, a, 0x001F); /* C0941B eor #$001F */
        S(0x941E, 2); t_write16(ss, dp + ptr_04, a);/* C0941E sta ptr_04 */
      }
      /* loc_C09420 */
      S(0x9420, 2);                         /* C09420 lda ptr_04 */
      a = t_read16(ss, dp + ptr_04); ss_set_nz16(ss, a);
      S(0x9422, 3); a = alu_and16(ss, a, 0x000F);   /* C09422 and #$000F */
    }
    ...
    S(0x942A, 2); t_write16(ss, dp + 0x08, a);      /* C0942A sta $08 */
    ...
    S(0x9431, 2); t_write16(ss, dp + ptr_04, a);    /* C09431 sta ptr_04 */
    ...
    S(0x9468, 3); a = alu_adc16(ss, a, 0x0000);     /* C09468 adc #$0000 */
    ...
    S(0x9491, 3); alu_cmp16(ss, a, 0x00F0); /* C09491 cmp #$00F0 */
    taken = ss_c(ss);
    ...
    a = alu_adc16(ss, a, t_read16(ss, dp + 0x06));
    S(0x94A5, 3); t_index(ss);              /* C094A5 sta ptr_04,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (ptr_04 + y)), a);

    S(0x94A8, 2);                           /* C094A8 lda $1E */
    a = t_read16(ss, dp + 0x1E); ss_set_nz16(ss, a);
    SI(0x94AA); ss_set_c(ss, true);         /* C094AA sec */
    S(0x94AB, 2);                           /* C094AB sbc $1A */
    a = alu_sbc16(ss, a, t_read16(ss, dp + 0x1A));
    S(0x94AD, 3); alu_cmp16(ss, a, 0x00F0); /* C094AD cmp #$00F0 */
    taken = ss_c(ss);
    ...
    a = alu_adc16(ss, a, t_read16(ss, dp + spc_dest_addr));
    S(0x94C1, 3); t_index(ss);              /* C094C1 sta $0005,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0005 + y)), a);

    S(0x94C4, 2);                           /* C094C4 lda $08 */
    a = t_read16(ss, dp + 0x08); ss_set_nz16(ss, a);
    ...
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0006 + y)), a);
    SI(0x94D2); a = y; ss_set_nz16(ss, a);  /* C094D2 tya */
    S(0x94D3, 3); a = alu_adc16(ss, a, 0x0008);     /* C094D3 adc #$0008 */
  ...
  next_slot2:                               /* loc_C094D7 */
    ...
    S(0x94DB, 3);                           /* C094DB jmp loc_C09348 */
  }
}
...
void sparkle_array_init(SnesState* ss) {
  ...
  S(0x94E4, 3); a = 0x00F8; ss_set_nz16(ss, a);   /* C094E4 lda #$00F8 */
  ...
  do {
    ...
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x94F2, 3); a = alu_and16(ss, a, 0x07FF);   /* C094F2 and #$07FF */
    S(0x94F5, 4);                           /* C094F5 sta $7F0E87,X */
    t_write16(ss, (0x7F0E87 + x) & 0xffffff, a);

    S(0x94F9, 2);                           /* C094F9 lda init_magic_FFFF */
    a = t_read16(ss, dp + init_magic_FFFF); ss_set_nz16(ss, a);
    S(0x94FB, 3); a = alu_and16(ss, a, 0x007F);   /* C094FB and #$007F */
    S(0x94FE, 3); a = alu_adc16(ss, a, 0x0060);   /* C094FE adc #$0060 */
    S(0x9501, 4);                           /* C09501 sta $7F0E89,X */
    t_write16(ss, (0x7F0E89 + x) & 0xffffff, a);

    S(0x9505, 2);                           /* C09505 lda init_magic_FFFF */
    a = t_read16(ss, dp + init_magic_FFFF); ss_set_nz16(ss, a);
    S(0x9507, 3); a = alu_and16(ss, a, 0x007F);   /* C09507 and #$007F */
    S(0x950A, 3); a = alu_adc16(ss, a, 0x0040);   /* C0950A adc #$0040 */
    S(0x950D, 2);                           /* C0950D bit init_magic_FFFF */
    alu_bit16(ss, a, t_read16(ss, dp + init_magic_FFFF));
    taken = !ss_n(ss);
    S(0x950F, 1); t_branch(ss, taken);      /* C0950F bpl loc_C09515 */
    if(!taken) {
      S(0x9511, 3); a = alu_eor16(ss, a, 0xFFFF); /* C09511 eor #$FFFF */
    ...
    S(0x951B, 3); a = alu_sbc16(ss, a, 0x0008);   /* C0951B sbc #$0008 */
    taken = !ss_n(ss);
...
void sparkle_update_and_draw(SnesState* ss) {
  ...
  y = t_read16(ss, dp + oam_write_ptr); ss_set_nz16(ss, y);
  S(0x9523, 3); alu_cpx16(ss, y, 0x0400);   /* C09523 cpy #$0400 */
  taken = !ss_c(ss);
  ...
  /* loc_C09529 */
  S(0x9529, 3); a = 0x00F8; ss_set_nz16(ss, a);   /* C09529 lda #$00F8 */
  ...
  for(;;) {
    ...
    S(0x9531, 2); t_write16(ss, dp + 0x06, a);    /* C09531 sta $06 */
    ...
    S(0x953C, 3); alu_cmp16(ss, a, 0x0100); /* C0953C cmp #$0100 */
    taken = ss_c(ss);
    ...
    t_write16(ss, ss_abs(ss, (uint16_t) (nmi_handler_ptr + y)), a);
    S(0x9544, 3); a = alu_adc16(ss, a, 0x0008);   /* C09544 adc #$0008 */
    S(0x9547, 2);                           /* C09547 bit $06 */
    alu_bit16(ss, a, t_read16(ss, dp + 0x06));
    taken = ss_n(ss);
    S(0x9549, 1); t_branch(ss, taken);      /* C09549 bmi loc_C0954E */
    if(!taken) {
      S(0x954B, 3); a = alu_adc16(ss, a, 0xFFF0); /* C0954B adc #$FFF0 */
    }
    ...
    a = t_read16(ss, (0x7F0E87 + x) & 0xffffff); ss_set_nz16(ss, a);
    S(0x9556, 3); a = alu_and16(ss, a, 0x001E);   /* C09556 and #$001E */
    ...
    S(0x956F, 3); a = alu_adc16(ss, a, 0x0040);   /* C0956F adc #$0040 */
    ...
    S(0x958F, 3); a = alu_adc16(ss, a, 0x0008);   /* C0958F adc #$0008 */
    SI(0x9592); y = a; ss_set_nz16(ss, y);  /* C09592 tay */

  integrate:                                /* loc_C09593 */
    S(0x9593, 2); t_write16(ss, dp + ptr_04, 0);  /* C09593 stz ptr_04 */
    ...
    if(!taken) {
      S(0x95B9, 3); a = alu_and16(ss, a, 0xFFFF); /* C095B9 and #$FFFF */
      taken = ss_n(ss);
      ...
      /* loc_C095C0 */
      S(0x95C0, 3); alu_cmp16(ss, a, 0x0580);     /* C095C0 cmp #$0580 */
      taken = !ss_c(ss);
    ...
    a = t_read16(ss, (0x7F0E8A + x) & 0xffffff); ss_set_nz16(ss, a);
    S(0x95C9, 3); a = alu_eor16(ss, a, 0xFFFF);   /* C095C9 eor #$FFFF */
    SI(0x95CC); a = alu_inc16(ss, a);             /* C095CC inc A */
    S(0x95CD, 4);                           /* C095CD sta $7F0E8A,X */
    t_write16(ss, (0x7F0E8A + x) & 0xffffff, a);

  advance:                                  /* loc_C095D1 */
    S(0x95D1, 3); alu_cpx16(ss, y, 0x0400); /* C095D1 cpy #$0400 */
    taken = ss_c(ss);
    ...
    S(0x95D8, 3); a = alu_sbc16(ss, a, 0x0008);   /* C095D8 sbc #$0008 */
    taken = ss_n(ss);
    S(0x95DB, 1); t_branch(ss, taken);      /* C095DB bmi loc_C095E0 */
    if(taken) break;
    S(0x95DD, 3);                           /* C095DD jmp loc_C0952C */
  }

  /* loc_C095E0 */
  S(0x95E0, 2); t_write16(ss, dp + oam_write_ptr, y);  /* C095E0 sty oam_write_ptr */
...
void particle_spawn_from_table(SnesState* ss) {
  ...
  S(0x95E3, 3); a = 0x1F00; ss_set_nz16(ss, a);   /* C095E3 lda #$1F00 */
  ...
  a = t_read16(ss, dp + game_mode); ss_set_nz16(ss, a);
  ...
  S(0x95F6, 3); y = 0x003A; ss_set_nz16(ss, y);   /* C095F6 ldy #$003A */
  ...
  for(;;) {
    ...
    S(0x9600, 3); a = alu_sbc16(ss, a, 0x0080);   /* C09600 sbc #$0080 */
    S(0x9603, 3); t_index(ss);              /* C09603 sta $0A06,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0A06 + y)), a);
    ...
    S(0x960B, 3); a = alu_sbc16(ss, a, 0x0080);   /* C0960B sbc #$0080 */
    S(0x960E, 3); t_index(ss);              /* C0960E sta $0A86,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0A86 + y)), a);

    S(0x9611, 3); a = 0x8000; ss_set_nz16(ss, a); /* C09611 lda #$8000 */
    S(0x9614, 3); t_index(ss);              /* C09614 sta $0B06,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0B06 + y)), a);
    S(0x9617, 3); t_index(ss);              /* C09617 sta $0B86,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0B86 + y)), a);

    JSRC(0x961A, 0xA212, random_next);      /* C0961A jsr random_next */
    S(0x961D, 2);                           /* C0961D lda init_magic_AA55 */
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x961F, 3); a = alu_and16(ss, a, 0x1FFF);   /* C0961F and #$1FFF */
    S(0x9622, 3); t_index(ss);              /* C09622 sta $0C06,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0C06 + y)), a);

    JSRC(0x9625, 0xA212, random_next);      /* C09625 jsr random_next */
    S(0x9628, 2);                           /* C09628 lda init_magic_AA55 */
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x962A, 3); a = alu_and16(ss, a, 0x01FF);   /* C0962A and #$01FF */
    SI(0x962D); ss_set_c(ss, false);              /* C0962D clc */
    S(0x962E, 3); a = alu_adc16(ss, a, 0x0100);   /* C0962E adc #$0100 */
    S(0x9631, 3); t_index(ss);              /* C09631 sta $0C86,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0C86 + y)), a);

    JSRC(0x9634, 0xA212, random_next);      /* C09634 jsr random_next */
    S(0x9637, 2);                           /* C09637 lda init_magic_AA55 */
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x9639, 3); a = alu_and16(ss, a, 0x01FF);   /* C09639 and #$01FF */
    S(0x963C, 3); a = alu_adc16(ss, a, 0x0100);   /* C0963C adc #$0100 */
    S(0x963F, 3); t_index(ss);              /* C0963F sta $0D06,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0D06 + y)), a);

    SI(0x9642); a = ss_dp(ss); ss_set_nz16(ss, a);/* C09642 tdc */
    S(0x9643, 3); t_index(ss);              /* C09643 sta $0D86,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0D86 + y)), a);
    S(0x9646, 3); t_index(ss);              /* C09646 sta $0E06,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0E06 + y)), a);

    JSRC(0x9649, 0xA212, random_next);      /* C09649 jsr random_next */
    S(0x964C, 2);                           /* C0964C lda init_magic_AA55 */
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x964E, 3); a = alu_and16(ss, a, 0x00FF);   /* C0964E and #$00FF */
    S(0x9651, 3); a = alu_adc16(ss, a, 0x0080);   /* C09651 adc #$0080 */
    S(0x9654, 3); t_index(ss);              /* C09654 sta $0906,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0906 + y)), a);

    S(0x9657, 2);                           /* C09657 lda init_magic_FFFF */
    a = t_read16(ss, dp + init_magic_FFFF); ss_set_nz16(ss, a);
    S(0x9659, 3); a = alu_and16(ss, a, 0x007F);   /* C09659 and #$007F */
    S(0x965C, 3); a = alu_adc16(ss, a, 0x0080);   /* C0965C adc #$0080 */
    S(0x965F, 3); t_index(ss);              /* C0965F sta $0986,Y */
    t_write16(ss, ss_abs(ss, (uint16_t) (0x0986 + y)), a);
  ...
  if(!taken) {
    S(0x966D, 3); a = 0xFFFF; ss_set_nz16(ss, a); /* C0966D lda #$FFFF */
    do {
      /* loc_C09670 */
...
void particle_spawn_random(SnesState* ss) {
  ...
  S(0x9781, 3); a = 0x1F00; ss_set_nz16(ss, a);   /* C09781 lda #$1F00 */
  ...
  S(0x9787, 3); x = 0x003A; ss_set_nz16(ss, x);   /* C09787 ldx #$003A */
  ...
  do {
    ...
    a = t_read16(ss, dp + init_magic_AA55); ss_set_nz16(ss, a);
    S(0x978F, 3); a = alu_and16(ss, a, 0x07FF);   /* C0978F and #$07FF */
    S(0x9792, 4);                           /* C09792 sta $7F0A06,X */
    t_write16(ss, (0x7F0A06 + x) & 0xffffff, a);

    S(0x9796, 2);                           /* C09796 lda init_magic_FFFF */
    a = t_read16(ss, dp + init_magic_FFFF); ss_set_nz16(ss, a);
    S(0x9798, 3); a = alu_and16(ss, a, 0x003F);   /* C09798 and #$003F */
    S(0x979B, 3); a = alu_adc16(ss, a, 0x0060);   /* C0979B adc #$0060 */
    S(0x979E, 4);                           /* C0979E sta $7F0A86,X */
    t_write16(ss, (0x7F0A86 + x) & 0xffffff, a);

    S(0x97A2, 3); a = 0x8000; ss_set_nz16(ss, a); /* C097A2 lda #$8000 */
    S(0x97A5, 4);                           /* C097A5 sta $7F0B06,X */
    t_write16(ss, (0x7F0B06 + x) & 0xffffff, a);
    S(0x97A9, 4);                           /* C097A9 sta $7F0B86,X */
    t_write16(ss, (0x7F0B86 + x) & 0xffffff, a);

    S(0x97AD, 3); a = 0xFC00; ss_set_nz16(ss, a); /* C097AD lda #$FC00 */
    S(0x97B0, 4);                           /* C097B0 sta $7F0C86,X */
    t_write16(ss, (0x7F0C86 + x) & 0xffffff, a);

    S(0x97B4, 3); a = 0x0800; ss_set_nz16(ss, a); /* C097B4 lda #$0800 */
    ...
    S(0x97C8, 3); a = 0x0700; ss_set_nz16(ss, a); /* C097C8 lda #$0700 */
    S(0x97CB, 4);                           /* C097CB sta $7F0C06,X */
    t_write16(ss, (0x7F0C06 + x) & 0xffffff, a);

    S(0x97CF, 2);                           /* C097CF lda init_magic_FFFF */
    a = t_read16(ss, dp + init_magic_FFFF); ss_set_nz16(ss, a);
    S(0x97D1, 3); a = alu_and16(ss, a, 0x00FF);   /* C097D1 and #$00FF */
