...
| 3. Asset extraction with round-trip gates (sprites, tiles, maps, palettes, BRR, music) | in progress: `config/assets.txt` (1920 named assets, generated by `tools/gen_assets.py`) covers the whole ROM; `tools/extract.py` writes them under `data/`, `tools/emit_asar.py` incbins by name |
